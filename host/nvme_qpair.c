...
#ifdef NVME_PCIE_FPGA
	} else if (!nvme_qpair_is_admin_queue(qpair) && pqpair->sq_tail == pqpair->sq_head &&
		   !STAILQ_EMPTY(&qpair->queued_req)) {
		/* 避免多队列时由于没抢到锁, 导致sq中一直为空而无法resubmit */
		nvme_qpair_resubmit_requests(qpair, 1);
#endif
	} else {
		_nvme_qpair_complete_abort_queued_reqs(qpair);
	}
...
static void
nvme_admin_qpair_iolog_write(struct spdk_nvme_qpair *qpair, struct spdk_nvme_cmd *cmd)
{
	struct spdk_nvmf_capsule_cmd *fcmd = (void *)cmd;
	struct timespec ts;
	...
	case SPDK_NVME_OPC_SET_FEATURES:
	case SPDK_NVME_OPC_GET_FEATURES:
		fprintf(qpair->iolog_f, "%ld.%09ld %s %s command id:%d cdw10:%08x\n", 
				ts.tv_sec, ts.tv_nsec, nvme_get_string(admin_opcode, cmd->opc), nvme_get_string(feat_opcode,
				cmd->cdw10_bits.set_features.fid), cmd->cid, cmd->cdw10);
		break;
	case SPDK_NVME_OPC_FABRIC:
		fprintf(qpair->iolog_f, "%ld.%09ld %s %s qid:%d command id:%d\n", 
				ts.tv_sec, ts.tv_nsec, nvme_get_string(admin_opcode, cmd->opc), nvme_get_string(fabric_opcode, fcmd->fctype), 
				qpair->id, fcmd->cid);
		break;
	default:
		fprintf(qpair->iolog_f, "%ld.%09ld %s (%02x) qid:%d command id:%d nsid:%x cdw10:%08x cdw11:%08x\n", 
				ts.tv_sec, ts.tv_nsec, nvme_get_string(admin_opcode, cmd->opc), 
				cmd->opc, qpair->id, cmd->cid, cmd->nsid, cmd->cdw10, cmd->cdw11);
	}
}

static void
nvme_io_qpair_iolog_write(struct spdk_nvme_qpair *qpair, struct spdk_nvme_cmd *cmd)
{
	struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);
	...
	case SPDK_NVME_OPC_WRITE_UNCORRECTABLE:
	case SPDK_NVME_OPC_COMPARE:
		fprintf(qpair->iolog_f, "%ld.%09ld %s sqid:%d command id:0x%04x nsid:%d slba:%llu nlb:%d\n", 
			ts.tv_sec, ts.tv_nsec, nvme_get_string(io_opcode, cmd->opc), qpair->id, cmd->cid, cmd->nsid,
			((unsigned long long)cmd->cdw11 << 32) + cmd->cdw10,
			(cmd->cdw12 & 0xFFFF) + 1);
		break;
	case SPDK_NVME_OPC_FLUSH:
	case SPDK_NVME_OPC_DATASET_MANAGEMENT:
		fprintf(qpair->iolog_f, "%ld.%09ld %s sqid:%d command id:0x%04x nsid:%d\n", 
			ts.tv_sec, ts.tv_nsec, nvme_get_string(io_opcode, cmd->opc), qpair->id, cmd->cid, cmd->nsid);
		break;
	default:
		fprintf(qpair->iolog_f, "%ld.%09ld %s (%02x) sqid:%d command id:0x%04x nsid:%d\n", 
			ts.tv_sec, ts.tv_nsec, nvme_get_string(io_opcode, cmd->opc), cmd->opc, qpair->id, cmd->cid, cmd->nsid);
		break;
	}
}

static void
spdk_nvme_write_iolog(struct spdk_nvme_qpair *qpair, struct spdk_nvme_cmd *cmd)
{
	assert(cmd != NULL);

	if (qpair->id == 0 || cmd->opc == SPDK_NVME_OPC_FABRIC) {
		nvme_admin_qpair_iolog_write(qpair, cmd);
	} else {
		nvme_io_qpair_iolog_write(qpair, cmd);
	}
}

static int
nvme_qpair_init_iolog(struct spdk_nvme_qpair *qpair)
{
	char filename[256];

	snprintf(filename, sizeof(filename), "%s_qid_%d_iolog", qpair->ctrlr->trid.subnqn, qpair->id);
	...
	printf("create iolog file:%s\n", filename);

	qpair->iolog_f = f;
	qpair->iolog_buf = spdk_malloc(8192, 0x1000, NULL,
							SPDK_ENV_LCORE_ID_ANY, SPDK_MALLOC_DMA);
	setvbuf(f, qpair->iolog_buf, _IOFBF, 8192);

	if (fprintf(f, "nqn [%s] qpair id [%d] iolog, ", qpair->ctrlr->trid.subnqn, qpair->id) < 0) {
		perror("iolog init\n");
		return -ENOMEM;
	}
	fprintf(f, "%s\n", qpair->id == 0 ? "Admin qpair" : "IO qpair");

	return 0;
}

static void
nvme_qpair_deinit_iolog(struct spdk_nvme_qpair *qpair)
{
	printf("free iolog resource\n");
	if (!qpair->iolog_f) return;
	
	fflush(qpair->iolog_f);
	fclose(qpair->iolog_f);
	spdk_free(qpair->iolog_buf);
	qpair->iolog_f = NULL;
	qpair->iolog_buf = NULL;
	printf("free iolog resource success\n");
}
...
// #ifdef IO_LOG
	rc = nvme_qpair_init_iolog(qpair);
	if(rc) return rc;
// #endif
...
// #ifdef IO_LOG
if (rc == 0) {
	spdk_nvme_write_iolog(qpair, &req->cmd);
}
// #endif
	if (spdk_likely(rc == 0)) {
